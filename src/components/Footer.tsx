...
const DEFAULT_DESC =
  "Welcome to our store. We believe in providing the best quality products for our customers.";
const DEFAULT_PHONE = "";
...
function MastercardIcon() {
  return (
    <svg viewBox="0 0 24 24" className="footer__payment-icon">
      ...
      <path
        d="M12 7.2c1.15.95 1.9 2.45 1.9 4.05s-.75 3.1-1.9 4.05c-1.15-.95-1.9-2.45-1.9-4.05s.75-3.1 1.9-4.05z"
        fill="#FF5F00"
      />
    </svg>
  );
...
export default function Footer({
  ...
  const getInitialLogo = () => {
    ...
    return (
      ...
      headerStyle?.logoUrl ||
      initialCustomization?.headerConfig?.logoUrl ||
      DEFAULT_LOGO
    );
  };
  ...
  const getInitialStoreName = () => {
    ...
    return (
      ...
      initialCustomization?.headerConfig?.storeName ||
      propStoreName ||
      DEFAULT_NAME
    );
  };
    ...
    fetchPages(storeSubdomain)
      .then((pages) => {
        ...
        const customPages = pages.filter(
          (p) => !DEFAULT_SLUGS.includes(p.slug) && p.isActive,
        );
        const finalLinks = [
          ...updatedLinks,
          ...customPages.map((cp) => ({
      ...
      if (e.data?.type === "ORBIT_CUSTOMIZATION_UPDATE") {
        ...
        if (cust?.footerContent) {
          ...
          setContactInfo({
            ...
            email:
              fc?.contact?.email || cust.contactInfo?.email || DEFAULT_EMAIL,
            address:
              fc?.contact?.address ||
              cust.contactInfo?.address ||
              DEFAULT_ADDRESS,
          });
          setSocialLinks({
            facebook:
              fc?.socials?.facebook || cust.socialLinks?.facebook || DEFAULT_FB,
            instagram:
              fc?.socials?.instagram ||
              cust.socialLinks?.instagram ||
              DEFAULT_IG,
            twitter:
              fc?.socials?.twitter ||
              cust.socialLinks?.twitter ||
              DEFAULT_TWITTER,
            tiktok:
              fc?.socials?.tiktok || cust.socialLinks?.tiktok || DEFAULT_TIKTOK,
          });
      ...
      <div className="footer__row4">
        <div className="footer__payment-icons">
          ...
          <Image
            ...
            width={40}
            height={25}
            className="footer__payment-icon footer__payment-icon--upi"
            unoptimized
          />
          <Image
            src="/PhonePe.svg"
            ...
            width={40}
            height={25}
            className="footer__payment-icon footer__payment-icon--phonepe"
            unoptimized
          />
          <Image
            src="/RuPay.svg"
            ...
            width={40}
            height={25}
            className="footer__payment-icon footer__payment-icon--rupay"
            unoptimized
          />
        </div>
        <button
